async function startPorcupine(accessKey, sensitivity) {
    if(typeof PorcupineWebFrWorker == "undefined" ||
       typeof WebVoiceProcessor == "undefined"){
        setTimeout(function(){
            startPorcupine(accessKey, sensitivity);
        }, 100);
    }
    else{
...
MAIN.Socket.on("set_porcupine_settings", function(_settings) {
  if(_settings.Key != null){
    if(settings == null){
      settings = _settings;
      startPorcupine(settings.Key, settings.Sensitivity);
    }
  }
  else{
...
MAIN.Socket.emit("get_porcupine_settings");
